...
****
If you are sending data from {ls} to {serverless-full}, note these differences from {ess} and self-managed {es}:

* {serverless-full} uses data streams and Data Lifecycle Management (DLM) instead of Index Lifecycle Management (ILM). 
...
as logs, metrics, and events) into {es} and {serverless-full}:
...
[NOTE]
--
* The ILM feature does not apply for {serverless-full}. 
