...
This plugin can be configured to send compressed bulk requests to Elasticsearch, and will always read compressed responses.

If you are concerned about bandwidth, enabling <<plugins-{type}s-{plugin}-http_compression>> can allow you to trade a small amount of CPU capacity for a significant reduction in network IO.
...
Note that this plugin is capable of reading compressed _responses_ from Elasticsearch regardless of the value of this setting.
