...
| <<plugins-{type}s-{plugin}-user_agent>> |<<string,string>>|No
...
[id="plugins-{type}s-{plugin}-user_agent"]
===== `user_agent`

* Value type is <<string,string>>
* Default is `LS <Logstash version>-output <plugin version>`.

Defines the HTTP's `user-agent` header to be used in the comunication with Elasticsearch.
If not specified has a default value composed of Logstash and plugin's versions, for example: `LS 7.14.2-output 11.6.0`.
If specified the value provided is used as `user-agent` header's value.
