...
#
    ...
    def ssl_options
      @options[:client_settings] ? @options[:client_settings][:ssl] : {}
    end
    ...
    def build_pool(options)
      ...
      if self.user
        pool_options[:auth] = {:user => self.user, :password => self.password}
      end
    ...
    def host_to_url(h)
      ...
      uri.user = h.user if h.user
      uri.password = h.password if h.password
