...
describe LogStash::Outputs::ElasticSearch::HttpClient::Pool do
  ...
  describe "license checking with ElasticSearchOutputLicenseChecker" do
    ...
    context "when using default logstash distribution" do
      ...
      [ "8.0.0", "7.11.0" ].each do |es_node_version|
        context "ES #{es_node_version}" do
          let(:es_node_versions) { [ es_node_version ] }
      ...
      context "ES 7.10.2" do
        let(:es_node_versions) { [ "7.0.0" ] }

        context "if ES returns no license" do
          let(:license_status) { nil }
